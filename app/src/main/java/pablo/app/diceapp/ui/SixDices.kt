...
@Composable
fun SixDices(viewModel: MainViewModel, mediaPlayer: MediaPlayer, isOnPortraitMode: Boolean = true) {
    if(isOnPortraitMode) {
        Row {
            Image(
        ...
        Row {
            ...
            Image(
                painter = painterResource(id = viewModel.getImage(viewModel.sixthDice.number.value).value),
                contentDescription = viewModel.sixthDice.number.toString(),
                modifier = Modifier
                    .clickable {
                        viewModel.rollDice(viewModel.sixthDice.number)
                        mediaPlayer.start()
                    }
                    .size(150.dp)
            )
